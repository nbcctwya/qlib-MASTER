...
def run_ensemble_backtest(market: str, score: pd.Series, config: dict) -> pd.DataFrame:
    import qlib
    from qlib.backtest import backtest

    qlib.init(**config["qlib_init"])
    port_config = copy.deepcopy(config["port_analysis_config"])
    port_config["strategy"]["kwargs"]["signal"] = score.to_frame()
    executor = {
    ...
    portfolio, _ = backtest(
        executor=executor,
        strategy=port_config["strategy"],
        **port_config["backtest"],
    )
...
def main() -> None:
    ...
    for market in sorted(runs):
        ...
        for seed, run_dir in sorted(runs[market].items()):
            ...
            row.update(portfolio_metrics(report["return"] - report["cost"]))
    ...
    if ensemble_enabled:
        ...
        for market in sorted(runs):
            ...
            curve = run_ensemble_backtest(market, score, configs[market])
    ...
    for market, config in configs.items():
        ...
        market_details[market] = {
            ...
            "transaction_cost": backtest["exchange_kwargs"],
            "data_provider": config["qlib_init"]["provider_uri"],
    ...
    eval_config = {
        ...
        "strategy": {"class": "TopkDropoutStrategy", "topk": 30, "n_drop": 5, "weighting": "Qlib TopkDropoutStrategy default"},
